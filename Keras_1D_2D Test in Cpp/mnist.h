...
void read_mnist_char_label(char *file_path, int num_data, int len_info, int arr_n, unsigned char data_char[][1], int info_arr[])
{
    int i, j, k, fd;
    unsigned char *ptr;
...
void load_mnist()
{
    read_mnist_char(strdup(TRAIN_IMAGE), NUM_TRAIN, LEN_INFO_IMAGE, SIZE, train_image_char, info_image);
    read_mnist_char(strdup(TRAIN_IMAGE), NUM_TRAIN, LEN_INFO_IMAGE, SIZE, train_image_char, info_image);
    image_char2double(NUM_TRAIN, train_image_char, train_image);


    read_mnist_char(strdup(TEST_IMAGE), NUM_TEST, LEN_INFO_IMAGE, SIZE, test_image_char, info_image);
    image_char2double(NUM_TEST, test_image_char, test_image);

    read_mnist_char_label(strdup(TRAIN_LABEL), NUM_TRAIN, LEN_INFO_LABEL, 1, train_label_char, info_label);
    label_char2int(NUM_TRAIN, train_label_char, train_label);

    read_mnist_char_label(strdup(TEST_LABEL), NUM_TEST, LEN_INFO_LABEL, 1, test_label_char, info_label);
